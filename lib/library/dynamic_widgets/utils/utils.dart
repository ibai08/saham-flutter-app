...
MaterialTapTargetSize? parseTapTargetSize(String? tapTargetSize) {
  ...
  switch(tapTargetSize) {
    case "padded":
      values = MaterialTapTargetSize.padded;
      break;
    case "shrinkWrap":
      values = MaterialTapTargetSize.shrinkWrap;
      break;
    default: 
      values = null;
  }
...
Radius parseRadius(String radius) {
  final values = radius;
  double numberValues = double.parse(values);
  if (values.isNotEmpty) {
    return Radius.circular(double.parse(values));
  } else {
    return Radius.zero;
  }
}
...
BoxBorder? parseBoxBorderAll(Map<String, dynamic> map) {

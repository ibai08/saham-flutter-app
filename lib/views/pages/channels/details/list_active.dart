...
import '../../../../controller/list_active_controller_old.dart';
...
class ListActiveSignal extends StatelessWidget {
  ...
  List<Widget> getSignals(List<SignalInfo> cc) {
    ...
    cc.forEach((signal) {
      ...
      DateTime created =
          DateTime.parse(signal.createdAt!).add(const Duration(hours: 7));
      String expiredDate =
      ...
      String openTime = DateFormat('dd MMM yyyy HH:mm')
              .format(DateTime.parse(validOpenTime).add(const Duration(hours: 7))) +
          " WIB";
      if (signal.expired == 0) {
